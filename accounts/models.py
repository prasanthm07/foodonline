...
class UserManager(BaseUserManager):
    def create_user(self,first_name,last_name,username,email,password=None):
        if not email:
            raise ValueError("user must have enter the mail id")
        if not username:
...
class UserProfile(models.Model):
    ...
    profie_picture=models.ImageField(upload_to='users/profile_picture',blank=True)
    ...
    longtude=models.CharField(max_length=50,blank=True,null=True)
